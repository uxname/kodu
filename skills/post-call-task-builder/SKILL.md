...
description: Create structured technical specifications and development tasks from meeting transcripts OR refine and organize user-provided task lists. Use after team meetings, planning sessions, architecture discussions, or when user provides a raw task list that needs structuring, prioritization, and validation against the codebase.
---
...
Если пользователь дал транскрипцию / ссылку на созвон → режим A.
Если пользователь дал готовый список задач → режим B.
Если дано и то, и другое — уточни, с чем работать в первую очередь.
...
- цель;
- контекст;
- текущую проблему (для режима A) или исходный список (для режима B);
