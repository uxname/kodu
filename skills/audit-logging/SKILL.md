...
|----------|----------|--------|----------------|---------|
| LOG-01 | Production-код не использует console.log/console.error напрямую | ✅ PASS | — | — |
| LOG-02 | PII не логируется (email, телефон, имена, адреса, финансовые данные) | ❌ FAIL 🔴 | `auth/login.ts:34` | **1. Удалить email из лога, логировать только userId** \\ 2. Маскировать PII через log sanitizer \\ 3. Заменить на структурированный лог без PII |
| LOG-04 | Запросы трассируются (Request ID или correlation ID сквозной) | ⏸ ACCEPTED | — | В baseline: трейсинг через внешний сервис |
