...
**4. Оглавление:** Ссылки на `features.md` и `technical_reference.md`. Оглавление должно явно указывать ключевые
   технические секции (Архитектура и диаграмма, Безопасность, ADR, API-контракт) — чтобы читатель с первой страницы
...
Два обязательных подраздела:

*1.1 Компоненты и связи* — текстовое или ASCII-описание: кто с кем общается, какие компоненты существуют.

*1.2 Диаграмма потока данных* — Mermaid-блок сразу после описания компонентов, пока контекст свеж.
* Тип: `sequenceDiagram` или `flowchart TD`.
...
3. Сгенерируй `README.md` строго в порядке секций 1-6:
   ...
   * Секция 4 (оглавление): ссылки на features.md и ключевые секции technical_reference.md.
...
5. Сгенерируй `technical_reference.md` строго в порядке секций 1-8:
   * Секция 1 (архитектура):
     ...
     - 1.2: **сразу после** — Mermaid-диаграмма (happy path, ≤15 узлов). Тип выбирай:
       `sequenceDiagram` для запрос-ответных систем, `flowchart TD` для пайплайнов.
...
- [ ] Секция 1: содержит подсекции 1.1 (компоненты) и 1.2 (Mermaid-диаграмма)
- [ ] Секция 1.2: диаграмма умещается на один экран, показывает только happy path
