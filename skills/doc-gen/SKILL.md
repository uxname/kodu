...
- Если артефакт ещё не создан — указать в документе явно: `[TODO: создать]`
...
Команда `generate` автоматически создаёт папку `3_ARTIFACTS/` и подпапки `legal/`, `media/images/`, `media/video/`, `content/`, `examples/`. Если артефакты не нужны для данного проекта — папку можно удалить, но раздел `## Артефакты` в SPEC.md **должен присутствовать** с явной пометкой: «Артефактов нет.»
...
Пока файл не создан в `3_ARTIFACTS/` — в SPEC.md его не упоминать. Раздел `## Артефакты` должен содержать только уже существующие ссылки или «Артефактов нет.». Валидатор проверяет обратное: если файл СУЩЕСТВУЕТ в `3_ARTIFACTS/`, он обязан быть упомянут в документации. Ссылка без файла допустима технически (markdown не проверяет пути), но не рекомендуется — сначала создать файл, потом добавить ссылку.
