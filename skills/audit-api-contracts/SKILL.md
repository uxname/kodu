...
|----------|----------|--------|----------------|---------|
| API-01 | Форма ответов консистентна — единый envelope или его отсутствие по всему API | ✅ PASS | — | — |
| API-02 | HTTP статус-коды семантически корректны (201 при создании, 4xx для client errors) | ❌ FAIL 🟠 | `routes/auth.ts:78` | **1. Заменить res.status(200) на res.status(400) для ошибок валидации** \\ 2. Добавить централизованный error middleware \\ 3. Использовать HTTP Exception классы (HttpException) |
| API-07 | Публичный API имеет стратегию версионирования | ⏸ ACCEPTED | — | В baseline: внутренний API без внешних клиентов |
