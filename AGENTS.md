...
This repository ships the `koda` CLI (formerly PrintProject) and expects agents to treat it as a fast, bun-powered directory summarizer.
...
- When you add a test suite, prefer Bun’s built-in runner (e.g., `bun test src/commands/strip.test.ts`) so the test harness matches the runtime.
- To exercise a single spec once you add one, run `bun test path/to/test.ts` or include Bun’s `--filter`/`--testNamePattern` flags; this sticks with the `bun` tooling that already drives the CLI.
...
- When introducing tests, put them next to the feature they cover (e.g., `src/commands/strip.test.ts`) and keep them fast; we rely on Bun’s runner, so avoid slow setup scripts.
...
- History is stored via `~/.config/koda/history.json` (see `src/cli/history.ts`). Update that file atomically by reading via `loadHistory`, mutating the array, and calling `saveHistory` with `JSON.stringify`.
...
- `getPriorityScore` in `src/core/filter.ts` walks through `PRIORITY_RULES` and returns the first matching rule’s `score`, so high-priority files (docs, manifests) sort earlier in `generateOutputContent`.
...
- `DEFAULT_CONFIG.EXCLUDED_DIRS`, `EXCLUDED_FILES`, and `EXCLUDED_PATTERNS` live inside `src/config/defaults.ts`; keep these sets in sync with the README’s default exclusions when new patterns become relevant.
...
- `collectFilesRecursively` uses `relative(rootPath, fullPath).replace(/\\/g, '/')` so the CLI works consistently on Windows; don’t revert that normalization when touching paths.
...
## History Storage
- `HISTORY_FILE` lives at `~/.config/koda/history.json` and is created lazily via `ensureHistoryFileExists` before every read or write.
...
- When you document new defaults or exclusions, update both `src/config/defaults.ts` and the README `## 🚫 Default Exclusions` section to avoid confusion.
- Use the README’s `### ⚙️ Options & Flags` tables as the ground truth for what options exist and how they map to CLI flags.

## Release & Publishing
- `bun run prepublishOnly` already runs `bun run build`, so rely on it whenever you’re preparing a release tarball.
- The published package only includes `dist`, `README.md`, and `LICENSE`; make sure generated assets stay under `dist` and nothing else sneaks into `package.json`’s `files` array.
...
- History operations that fail to write log an error but otherwise continue; if `~/.config/koda` is writable then history should rebuild automatically.
...
- `bun.lock` and `dist/` are excluded from the npm package via `package.json`’s `files` array, so build artifacts never slip into published packages.
...
- When you touch CLI behavior, validate manual scenarios (`koda`, `koda strip`, `koda strip -y`) and consider adding fixture directories that mimic README descriptions so future agents can test them automatically.
