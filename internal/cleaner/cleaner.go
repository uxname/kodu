...
// регуляркой.
//
// Отличие от TS: для .html НЕ выполняется попытка распарсить содержимое как
// TypeScript (в оригинале это побочно ловило `//` внутри URL и могло портить
// разметку). Удаляются только настоящие HTML-комментарии — это безопаснее.
package cleaner
